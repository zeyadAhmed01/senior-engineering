...
Keep tool output as small as the next decision permits. On native Windows, a released RTK binary may be invoked selectively for noisy commands when it materially reduces output; it is optional and needs no transparent hook. Skip it where measured benefit is negligible. Prefer non-verbose test/build commands. When a successful command emits progress or otherwise noisy output, redirect it to a temporary log and report only the command, scope, exit status, and relevant counts; do not stream the full output and summarize it afterward. On failure, inspect the compact result first; if failed names, warnings, stack frames, or causal diagnostics are missing, retrieve the relevant log with `rtk recall` or a focused raw read before diagnosing or claiming verification. Raw commands remain authoritative when needed. Use deterministic scripts for mechanical audits instead of loading raw listings into model context.
