...
For marketing or brand-heavy pages, use the project's brand system first, then Dribbble, then Pinterest if needed. No external service or paid access is mandatory. If authentication, MCP, service access, or relevant search results are unavailable, irrelevant, or poor, say so briefly and continue from the supplied brief and repository evidence. If neither source contributes, do not invent findings.
...
Map each accepted design criterion to an observable check: state/interaction tests, responsive viewport, keyboard and accessibility behavior, localized layout/content, or a visual comparison. Check light/dark and LTR/RTL variants when they are supported and affected. For proposal-only work that materially changes a workflow or interaction model, include a concise Design Contract with the user and outcome, current path and relevant states, key interaction/layout constraints, unresolved decisions, and acceptance evidence. For proposal-only visual work, or when implementation or inspection tools are unavailable, end the recommendation with an **Evidence and next check** line. State which local evidence supports the direction, which visual or accessibility checks were not run, and the smallest next observation (for example, inspect the affected payment states at a target viewport and exercise keyboard focus). Mark checks that were not run as unverified. Use the narrowest checks that cover the changed states; do not claim a visual or accessibility state that was not observed.
